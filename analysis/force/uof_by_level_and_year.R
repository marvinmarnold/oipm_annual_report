check.vars(c("year", "uof.all", "uof.2015"))

########################################################################################################
########################################################################################################
# Combine 2015 UOF with all other uof taken directly from IAPro
all.uof.with.lvl.type <- uof.all %>% select(Force.type, Force.level, Year.Occurred)
uof.2015.with.lvl.type <- uof.2015 %>% select(Force.type, Force.level, Year.Occurred)
all.uof.with.lvl.type <- rbind(all.uof.with.lvl.type, uof.2015.with.lvl.type)

all.uof.with.lvl.type <- all.uof.with.lvl.type %>% filter(Year.Occurred >= 2015, Year.Occurred <= year)
lvls <- c("L1", "L2", "L3", "L4")
p.lvl.by.year <- lapply(lvls, function (lvl) {
  
  # Filter UOF by year
  uof.for.lvl <- all.uof.with.lvl.type %>% filter(Force.level == lvl) %>% group_by(Year.Occurred, Force.type)
