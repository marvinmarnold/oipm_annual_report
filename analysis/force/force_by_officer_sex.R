...
title <- "Force by officer sex"

########################################################################################################
########################################################################################################
uof.by.sex <- uof2017 %>% group_by(Officer.sex)
count.by.sex <- summarize(uof.by.sex, num.uof = n())
count.by.sex

p <- plot_ly(count.by.sex, labels = ~Officer.sex, values = ~num.uof, type = 'pie',
             textposition = 'inside',
...
# Group UOF by type
uof.by.sex.type <- uof.by.sex %>% group_by(Officer.sex, Force.level) 
...
male.pct <- function(lvl) {
  ...
  count.for.lvl.male <- count.for.lvl %>% filter(Officer.sex == 'M')
...
p <- plot_ly(count.by.sex.type, x = ~Force.level, y = ~num.uof, type = 'bar',  
             name = ~Officer.sex, color = ~Officer.sex, 
